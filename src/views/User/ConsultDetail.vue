...
import ConsultMoreVue from './components/ConsultMore.vue'
  ...
  <div class="detail-page" v-if="item">
    ...
    <div class="detail-page-head">
      ...
      <div class="card">
        ...
        <p class="doc">
          <span>极速问诊</span>
          <span>{{ item.docInfo?.name || '自动分配医生' }}</span>
        </p>
        <van-icon name="arrow" />
      </div>
    </div>
    <div class="detail-page-patient">
    ...
    <div class="detail-page-con"></div>
    <div class="detail-page-order">
      <h3>订单信息</h3>
      <van-cell-group :border="false">
        <van-cell title="订单编号" class="cellFist">
          <template #value>
    ...
    <div class="detail-page-time" v-if="item.status === OrderType.ConsultPay">
      请在 <van-count-down :time="item.countdown * 1000" /> 内完成支付，超时订单将取消
    </div>
    <!-- 待支付 -->
    <div class="detail-page-action van-hairline--top" v-if="item.status === OrderType.ConsultPay">
      <div class="price">
        <span>需付款</span>
    ...
    <!-- 待沟通 -->
    <div class="detail-page-action van-hairline--top" v-if="item.status === OrderType.ConsultWait">
      <van-button type="default" round :loading="loading" @click="cancelConsultOrder(item!)"
        >取消问诊</van-button
      >
    ...
    <!-- 待查看处方 -->
    <div class="detail-page-action van-hairline--top" v-if="item.status === OrderType.ConsultChat">
      <van-button
        type="default"
        round
    ...
    <!-- 待评价 -->
    <div
      class="detail-page-action van-hairline--top"
